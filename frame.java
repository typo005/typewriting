...
public class frame extends JFrame implements ActionListener, KeyListener
{
	...
	public void draw(Graphics g)
	{
		...
		if(ended)
		{
			...
			g.drawString("Typing Test Completed!", (SCREEN_WIDTH-metrics.stringWidth("Typing Test Completed!"))/2, g.getFont().getSize()*5);
			
			g.setColor(Color.BLACK);
			g.drawString("Typing Speed: "+WPM+" Words Per Minute", (SCREEN_WIDTH-metrics.stringWidth("Typing Speed: "+WPM+" Words Per Minute"))/2, g.getFont().getSize()*8);
			
			
			g.setColor(Color.BLACK);
			g.drawString("Character per minute: "+CPM+" Character Per Minute", (SCREEN_WIDTH-metrics.stringWidth("Charater per min: "+CPM+" Charater Per Minute"))/2, g.getFont().getSize()*13);
			
			   
			g.setColor(Color.BLACK);
			g.drawString("Accuracy: "+((count-wrongtyped)/200)*100+" %", (SCREEN_WIDTH-metrics.stringWidth("Accuary: "+((count-wrongtyped)/200)*100+" %"))/2, g.getFont().getSize()*17);
	...
	public void keyTyped(KeyEvent e) //keyTyped uses the key Character which can identify capital and lowercase difference in keyPressed it takes unicode so it also considers shift which creates a problem
	{
		if(passage.length()>1)
		{
			...
			else if(count==200) //Once all 200 characters are typed we will end the time and calculate time elapsed//edifucktyed22222
			{
				end=LocalTime.now().toNanoOfDay();
				elapsed=end-start;
				seconds=elapsed/1000000000.0; //nano/1000000000.0 is seconds
				WPM=(int)(((200/5)/seconds)*60); //number of character by 5 is one word by seconds is words per second * 60 WPM
				CPM=WPM*5;
				ended=true;
				running=false;
				count++;
			...
			if(typed<200) //edifucktyed22222
			{
				...
				if(e.getKeyChar()==pass[typed]) 
				{
					...
					count++; 
				       ...
				       wrongtyped++; //If the typed character is not equal to the current position it will not add it to the typedPassage, so the user needs to type the right thing
